var Tracker = {
	...
	init : function() {
		Store.init();
		this.boards = Store.get('other.tracker.boards',['b','po','vg']);
		...
		this.getdata();
		window.t = window.setInterval(function() {Tracker.update()}, 1000);
		//add
		$('.box__data,.tracker').on('click', '#add-board', function() {
			...
			$(this).val('Добавлено!');
			var that = this;
			window.setTimeout(function() {$(that).val('Добавить');}, 1000);
		});
		//
		//del
		...
		$('.box__data,.tracker').on('click', '.update', function(e) {
			...
			var that = this;
			window.setTimeout(function() {$(that).html('Обновить (<span class="counter"></span>)');}, 1000);
			return false;
		});
		//
		//hide
	...
	add: function(board) {
		...
		$('.boards-list').append('<span style="color:' + this.colors[this.colors.length - 1] + '" class="board-' + board + '"> / ' + board + '  <a href="#" data-board="' + board + '">x</a></span>');
		Store.set('other.tracker.boards', this.boards);
	},
	del: function(board) {
		var index = this.boards.indexOf(board);
	...
	getdata: function(callback){
		...
		var mode = this.mode;
		var that = this;
		var counter = 0;//код ниже выполняет итерации хоть и не асинхронно, но в разном порядке
		for(i = 0; i < boards_count; i++) {
			(function(i) {
				var current = that.boards[i];
				$.getJSON(
					'/' + current + '/catalog_num.json',
					function(data){
						//console.log(data)
						$.each(data.threads, function(index, thread) {
							thread.board = current; //mark thread <=> relations
							thread.color = that.colors[i]; //mark thread <=> color rel
	...
	display: function(output){
		...
		for(i = 0; i < boards.length; i++) {
			htmlboards += '<span style="color:' + this.colors[i] + '" class="board-' + boards[i] + '"> / ' + boards[i] + '  <a href="#" data-board="' + boards[i] + '">x</a></span>';
		}
		if(this.mode == 'Full') {
			for(i = 0; i < limit; i++) {
				var hidden = false;
				var h = Store.get('board.' + output[i].board + '.hidden');
				if(h) hidden = output[i].num in h;
				htmldata += '<div class="row tsthread row_' + output[i].num + '">';

				htmldata += '<div class="reply tsthread__meta">';
				htmldata += 'Создан: ' + output[i].date + ' | Постов: ' + output[i].posts_count + ' | <a href="#" class="' + (hidden?'unhide':'hide') + '" data-num="' + output[i].num + '" data-board="' + output[i].board + '">' + (hidden?'Развернуть':'Скрыть') + '</a> | <a href="/' + output[i].board + '/res/' + output[i].num + '.html">Ответить</a>';
				htmldata += '</div>';

				htmldata += '<div class="thread tsthread__body" ' + (hidden?'style="display:none;"':'') + '>';
				htmldata += '<div class="img tsthread__img">';
				htmldata += '<a href="/' + output[i].board + '/res/' + output[i].num + '.html">';
				htmldata += '<img class="thumb tsthread__thumb" src="' + output[i].files[0].thumbnail + '" width="' + output[i].files[0].tn_width + '" height="' + output[i].files[0].tn_height + '">';
				htmldata += '</a>'
				htmldata += '</div>';
				...
				if(output[i].subject && 0) {
					htmldata += '<br><b>' + output[i].subject + (output[i].tags?tag:'') + '</b>';
				}
				htmldata += '</div>';
				htmldata += '<div class="comment tsthread__comment">';
				htmldata += '<span style="color:' + output[i].color + '">/' + output[i].board + '/</span>: ' + output[i].comment;
				htmldata += '</div>';
				htmldata += '</div>';
			...
			for(i = 0; i < limit; i++) {
				htmldata += '<div class="row">' + '<span style="color:' + output[i].color + '">/' + output[i].board + '/</span>' + ' - <a href="/' + output[i].board + '/res/' + output[i].num + '.html" target="_blank">' + output[i].subject + '</a>';
				htmldata += '<span class="date"> [ ' + output[i].date + ' ]</span></div>';
			}
		}
		$("#data").html(htmldata);
